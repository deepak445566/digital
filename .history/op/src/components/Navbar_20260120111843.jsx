...
function Navbar({ isHomePage = false }) {
  ...
  const [isCurrentlyHomePage, setIsCurrentlyHomePage] = useState(isHomePage);
  ...
  // Update isCurrentlyHomePage when route changes
  useEffect(() => {
    const isHomePath = location.pathname === '/';
    setIsCurrentlyHomePage(isHomePath);
    setIsScrolled(false);
  ...
  // Initialize Shery.js magnet effect
  useEffect(() => {
  ...
  // Handle click outside dropdown on desktop
  useEffect(() => {
  ...
  return (
    <>
      {/* Main Navbar */}
      <nav 
        className={`fixed top-4 lg:top-4 left-2 right-2 lg:left-4 lg:right-4 z-[9999] transition-all duration-300 ${
          isScrolled ? 'shadow-xl bg-black/90 backdrop-blur-sm rounded-full' : ''
        }`}
      >
        <div className={`w-full py-1 lg:px-8 lg:py-1 ${
          isCurrentlyHomePage && !isScrolled ? 'bg-transparent' : 'bg-black'
        } rounded-full transition-all duration-300 ${isScrolled ? 'scale-[0.98]' : ''}`}>
          <div className="flex items-center justify-between">
            {/* Logo Section */}
            <div className="flex items-center">
              <img src="/logo.jpg" className="h-15 w-15 lg:h-19 lg:w-19 rounded-full" alt="Logo"/>
              <div className="flex flex-col">
                <span className={`font-bold text-md lg:text-lg tracking-wider exo ${
                  isCurrentlyHomePage && !isScrolled ? 'text-white' : 'text-white'
                }`}>
                  Digital Express<sup className="text-xs">®</sup>
                </span>
                <span className={`text-md exo tracking-widest ${
                  isCurrentlyHomePage && !isScrolled ? 'text-gray-300' : 'text-gray-300'
                }`}>
                  India
                </span>
              </div>
            ...
            <div className='flex items-center gap-5 relative' ref={dropdownRef}>
                                  ...
                                  {item.subServices.map((service, index) => (
                                    <div 
                                      ...
                                      className={`p-2 rounded-lg cursor-pointer transition-all duration-200 ${
                                        activeSubDropdown === service.id ? 'bg-blue-50 border-l-2 border-blue-500' : 'bg-gray-50 hover:bg-gray-100'
                                      }`}
                                      ...
                                      <div className="mb-6">
                                        <div className="flex items-center mb-3">
                                          <div className={`w-4 h-4 rounded-full ${
                                            item.subServices.findIndex(s => s.id === activeSubDropdown) % 5 === 0 ? 'bg-blue-500' : 
                                            item.subServices.findIndex(s => s.id === activeSubDropdown) % 5 === 1 ? 'bg-green-500' : 
                                            item.subServices.findIndex(s => s.id === activeSubDropdown) % 5 === 2 ? 'bg-purple-500' : 
                                            item.subServices.findIndex(s => s.id === activeSubDropdown) % 5 === 3 ? 'bg-orange-500' : 'bg-pink-500'
                                          } mr-3`}></div>
              ...
              <div className="md:hidden">
                <button 
                  ...
                  className={`px-2 lg:px-5 py-2 text-md font-medium rounded-full transition-all duration-300 shadow-lg hover:shadow-xl magnet ${
                    isCurrentlyHomePage && !isScrolled 
                      ? 'text-white bg-[#383838]/80 hover:bg-[#383838]' 
                      : 'text-white bg-[#383838] hover:bg-[#444444]'
                  }`}
                >
                      ...
                      <h3 className="text-black font-extrabold text-4xl lg:text-6xl exo tracking-tight opacity-90 group-hover:opacity-100 transition-all duration-500 flex items-center">
                        ...
                        {item.hasDropdown && (
                          <span className={`ml-4 text-2xl transform transition-transform duration-300 ${
                            activeOverlayItem === item.title ? 'rotate-90' : ''
                          }`}>
                            ›
                          </span>
                        )}
